...
#!try analysis with smaller dataset
res_tableOE_sample <- sample_n(res_tableOE, 20)
...
res_tableOE_tb <- res_tableOE_sample %>%
...
save(ego, file="results/ego_SAMPLE20.rda")
write.csv(cluster_summary,"results/clusterProfiler_Mov10oe_SAMPLE20.csv")


#---------------------------
#Visualization of ORA

pdf("results/plots_SAMPLE20.pdf")    #opens pdf file to write to 

# Creating Dotplot
dotplot(ego, showCategory=10, title = "OR Analysis [sample size 20]")      #how can i find parameter names to manipulate graph..? args() returning ... no good documentation (?**)
...
write.csv(gseaKEGG_results, "results/gseaOE_kegg_SAMPLE20.csv", quote=F)
...
pdf("results/plots_GSEA_SAMPLE20.pdf")    #open pdf file to write to 
